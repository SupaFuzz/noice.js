...
/*
    old value setter
    it's synchronous and that ain't gonna work going forward
    new value setter will break this but it's gotta be done
    leaving here for reference.
    note dataStreamCallback is no longr supported so stuff like
    coreUIScannerInput is broke, but then again there should be
    better ways of handling per-char input than setting 'keydown'
    as the captureValueOn ... yadig?
set value(v){
    let vt = v;

    // normalize case here if we have the flag
    if ((this.normalizeUpperCase == true) && (this.isNotNull(vt))){
        vt = vt.toUpperCase();
    }

    // strip leading and trailing whitespace here if we have the flag
    if ((this.trimWhitespace == true) && this.isNotNull(vt)){
        vt = vt.trim();
    }

    if (this.hasAttribute('dataStreamCallback') && (this.dataStreamCallback instanceof Function) && (! (this.setFromMenu == true))){

        if (!((this.labelLocation == 'embed') && ((vt == this.label)))){
            let that = this;
            try {
                let tmp = this.dataStreamCallback(vt, this._value, that);
    ...
    }else if (this.hasAttribute('valueChangeCallback') && (this.valueChangeCallback instanceof Function)){


        // here's the thing. we shouldn't even be executing the value change callback
        // for toggling the embedded label
    ...
    if (this.undoable){ this.oldValue = this._value; }
    this._value = vt;

    if ((this.formElement instanceof Element) && (this.formElement.value != vt)){
        this.formElement.value = vt;
    }
...
setValue(v){
    ...
    // normalize case here if we have the flag
    if ((this.normalizeUpperCase == true) && (this.isNotNull(v))){
        v = v.toUpperCase();
    }
...
setup(){
    ...
    */
    ...
    /*
        LOH 7/28/21 @ 1357
        gotta get ready and help Victoria find an apartment
        what's needed here is a focusin eventhandler, that we can hook up to a
        focusCallback() ... in that way ... back in npamFormView ...
        we can manage the "tab to next field" more properly.

        well maybe ... there's gotta be some kinda timeout on the advance to
        next field action, and we need to catch focus in and block it or something
        granted, this seems like a mess as well. but we gotta do SOMETHING here.
    */
    let that = this;
...
set undoable(b){
    ...
    if (newState){
        ...
        /*
            LOH 2/11/21 @ 1739
            what we need to do in here is setup the button and the hook if it's true
            and to hide the button otherswise.

            we might need to yank in some stuff from scannerInput.js
            in terms of where the button goes.

            I'm not inclined to take it father than that unless it becomes needed
            we'll clean it up later
        */

        let btnContainer = this.setupButtonContainer();
        let btn = document.createElement('button');
        btn.className = 'btnUndo';
        this.undoButton = btn;

        btnContainer.appendChild(btn);

        // start it out hidden. The value setter will unhide it for us
        ...
        btn.addEventListener('click', function(evt){
            ...
            that.undoButton.style.visibility = 'hidden';
        });

    }else{
        if (this.undoButton instanceof Element){
            btn.style.visibility = 'hidden';
        }
    }
    this._undoable = newState;
}
/*
    old value
*/
...
set oldValue(v){
    this._oldValue = v;
    if ((this.undoable) && (this.undoButton instanceof Element)){
        this.undoButton.style.visibility = 'visible';
    }
}
resetOldValue(){
    this.oldValue = null;
...
/*
    ...
    some field types need adjascent buttons. This mutates the html to make a place for them
*/
setupButtonContainer(){
    let btnContainer = this.DOMElement.querySelector('div.fieldBtnContainer');
    if (! (btnContainer instanceof Element)){
        let fieldSection = this.DOMElement.querySelector('div.ncufeFieldSection');
        if (fieldSection instanceof Element){

            // mutate CSS to make room for it "2em" isn't gonna work if we need more than one button though
            ...
            let btnContainerReal = document.createElement('div');
            btnContainerReal.className = "fieldBtnContainer";

            // in ya go!
            fieldSection.appendChild(btnContainerReal);
            return(btnContainerReal);
        }else{
            return(null);
        }
    }else{
...
embedLabel(bool){
    ...
    */
    ...
    function embeddedLabelExitFunctionPointer(e){
        let tmp = (that.formElement instanceof Element)?that.formElement.value:that._value;
        if ((tmp == that.label) && (that.labelLocation == 'embed')){ that.value = null; }
    };
    ...
    if (bool === true){
        ...
        /*
            LOH 5/30/20 @ 1131
            the reason the formView change detection is fooled by this
            is that we're using the value getter down here to detect the
            embedded label. *EXTERNAL* getters of the value needs to see
            NULL when the embedded label has the value. Hence we must use
            a separate INTERNAL reference to do the safteydance.
        */
...
get html(){
    ...
    return(`
        <label id="${this.formElementLabelGUID}" for="${this.formElementGUID}" class="${this.labelClass}">${this.label}</label><div class="${this._fieldSectionClass}">
        <input type="checkbox" id="${this.formElementGUID}" name="${this.name}" ${(this.enable)?'enabled':'disabled'}></input></div>
    `);
}
