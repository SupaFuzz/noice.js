...
*/
fieldValueChange(fieldName, valNew, valOld){
    console.log(`[fieldValueChange (${fieldName})] [new]: ${valNew} [old] ${valOld}`);

    /*
        LOH 5/27/20 @ 2325
        next -> figure out how to handle managing changedFields
        after that -> toggling the save button
    */
}
...
/*
    ...
    NOTE: if you set a 'value' attribute in the config mode-tree for a field
          ...
          LOH 5/27/20 @ 1330
          next step is to hook in the change flag management routine
          then let's try the whole shebang out
*/
setFormModeProperties(mode){
    Object.keys(this.formElements).forEach(function(fieldName){
