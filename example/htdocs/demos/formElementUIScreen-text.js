...
*/
class formElementTextUIScreen extends noiceCoreUIScreen {
...
constructor(args, defaults, callback){
    super(
        ...
        noiceObjectCore.mergeClassDefaults({
            _version:       1,
            _className:     'formElementTextUIScreen',
            firstFocus:     true
        }, defaults),
        callback
    );
...
setup(){
    ...
    // make a dropdown
    let testMenu = new noiceCoreUIFormElementText({
        label:              'comments',
        rows:               8,
        columns:            40,
    ...
    //
    this.DOMElement.querySelector('#btnResize').addEventListener('click',function(e){
        if (e.target.textContent == 'resize'){
            testMenu.resize = 'both';
            e.target.textContent = 'disable resize';
        }else{
            testMenu.resize = 'none';
            e.target.textContent = 'resize';
        }
    });
