...
get html(){
    ...
    return(`
        ...
        ${this.fieldReference('saveButton')}
...
setup(){
    ...
    */
    ...
    /*
        LOH 5/6/2020 @ 2135
        there is obviously a lot more to do here.
        for instance, respecting display statuses and
        the current mode of the form (create, modify, readonly)
...
set formMode(m){
    ...
    /*
        ...
        fill in this scaffold to implement renderers for each of the formModes
        once we've got that nailed down, get back to working on the saveCallback
    */
    switch(m){
        case 'locked':
...
/*
    ...
    set default value on all fields based on mode
*/
setDefaultValues(mode){
    Object.keys(this.formElements).forEach(function(fieldName){
        /*
            LOH 5/26/20 @ 2352
            loop through all of the fields and set their default values for
            the fiven mode.

            I'm wondering if a separate function is truly necessary?
            maybe we could just set value in the modes tree and send it
            along with everything else?

            I'm kinda liking that idea actually ...
        */
    },this);
}
...
clickSaveButton(evt){
    ...
    /*
        ...
        so if you don't got one you'd betta call somebody henny.

        LOH 5/14/2020 @ 1710
        see also LOH # 487

        the scaffold below should handle executing the asynchronous callback
        to deal with save/updates. Rather than just blindly sending the whole
        object I'd like to send the formMode and the result of a complete record
        data extraction. To do that, we've first gotta get a handle on rendering
        the form mode ... off to line 487 ...
    */
