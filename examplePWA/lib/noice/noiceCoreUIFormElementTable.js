...
set width(v){
    this._width = v;
    if (this.DOMElement instanceof Element){ this.DOMElement.style.width = this._width; }
}
get height(){ return(this._height); }
set height(v){
    this._height= v;
    if (this.DOMElement instanceof Element){ this.DOMElement.style.height = this._height; }
}
...
addRow(row, updateFlag){
    ...
    if (this.hasOwnProperty('addRowCallback') && (this.addRowCallback instanceof Function)){
        let that = this;
        this.addRowCallback(this._rows[this._rows.length -1], that);
    }
...
moveColumn(columnName, index, edge){
    ...
    if (currentPosition < 0){
        ...
        if (this.debug){ console.log(`bad columnName: ${columnName}`); }
        return(false);
    }
    ...
    if (isNaN(newPosition) || (newPosition < 0) || (newPosition > (this.columns.length -1))){
        ...
        if (this.debug){ console.log(`bad position: ${idx} / ${newPosition}`); }
        return(false)
    }
...
handleRowSelect(evt){
    ...
    /*
        7/21/20 @ 2156
        this isn't going to work when the cell has html content
        row can't jut blindly be the parent of whatever is the target
        because the target is whatever is burried in the cell's html
        I think what we need to do is find out how to turn off bubbling

        UPDATE @ 2226
        fuck that shit, let's just traverse the parents until we hit a tr
        note to self -> embed child tables in cells ... ugh.
        there's gotta be a better way but still ...
    */
    let row = evt.target;
    ...
    if (row.dataset.selected == 'true'){
        ...
        // invoke rowDeselectCallback if we've got such a thing ...
        if (this.rowDeselectCallback instanceof Function){
            this.rowDeselectCallback(row, this);
        }
...
/*
    getColumnDOMContent(row, columnName)
*/
getColumnDOMContent(row, columnName){
    if (this.columnOrder.indexOf(columnName) >= 0){
        return(row.querySelectorAll('td')[this.columnOrder.indexOf(columnName)]);
    }else{
...
getColumnValue(row, columnName){
    if (this.columnOrder.indexOf(columnName) >= 0){
        /*
            if it's an onscreen column and it has
            valueGetter defined, use that, if not
            just go for good old textContent
        */
        if (this.columnIndex[columnName].hasOwnProperty('valueGetter') && (this.columnIndex[columnName].valueGetter instanceof Function)){
            return(this.columnIndex[columnName].valueGetter(row.querySelectorAll('td')[this.columnOrder.indexOf(columnName)]));
        }else{
                ...
                if (sortBy[i].descending){
                    ...
                    switch(sortBy[i].sortMode){
                        ...
                        case   'date':
                            ...
                            /*
                                RETURN HERE 7/21/20 @ 2113
                                this is where we need an alternate sort mode
                                that can be associated to the column definition
                                when we work that out, this is where the
                                alternate sort function would get called.
                            */
...
setup(){
    ...
    */
    ...
    // attributes that need to be re-established post render
