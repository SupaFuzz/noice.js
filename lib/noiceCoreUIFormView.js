    ...
    */
    this.DOMElement.querySelectorAll(`.${this.fieldReferenceClass}`).forEach(function(el){
        ...
        if (this._fieldReferences[el.id] == 'saveButton'){
            /*
                LOH 5/12/2020 @ 1640
                rethinking it ... I'm not really sure the save button needs to be a formElement sublass
                it doesn't have a value and is it *really* worth the time to design a bunch of passthrough
                shit for callbacks? meh. build one right here with the DOM api and a couple hardcoded
                callbacks and let's get this done.

                but UAH python tonight so imma make myself some quick diner
