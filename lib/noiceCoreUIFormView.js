...
*/
setFormModeProperties(mode){
    console.log(`setFormModeProperties: ${mode}`)
    Object.keys(this.formElements).forEach(function(fieldName){
    ...
    /*
        ...
        this means next steps:
            ...
            2) go back into setup() and set the default form mode, etc
                -> [fixed 6/2/2020 @ 1420] value capture on embed labels tricks change flag detector
                -> [fixed 6/2/2020 @ 1721]  setting default values on date fields like what?
            ...
            4) test loading record data
                ...
                LOH 6/2/2020 @ 1721
                date value setting works you've just gotta use YYYY-MM-DD on *.value
                next step is setting up proper record lifecycle
                (load, unload, modify, create->modify and exit-wo-change)

                Kelly dropped that Andrea and Clay will get married at the courthouse tomorrow
                on me. an hour ago. in text message. There were riots in H'ville last night.
                The president has gone full fascist (objective fact, no longer speculation and
                no longer a plausible matter of debate -- that's what's happening). We are careening
                toward all out martial law. A Republican occupation of America. no fuckin' shit.
                COVID-19 is still a thing. In fact not a damn thing has changed, and there will
                be a new and overwhelming outbreak any day now. I suspect it's already happening its
                just that we have this awful convinient rioting going on so --- oops! no attention
                paid to the thousands and thousands dying on ventillators all over the place.

                There's a reason I've made abysmal progress over the past week or two.
                I can't think anymore because I can't help but see what's going on around me
                and it is NEVER anything but a higher level of heartbreak and outrage than the
                day before and it's been that way since november 2016. No new day is an exception.
                And now we're to the point where *no new minute* is an exception (literally).

                Evil is beating us. Decisively. Hope is getting damn hard to come by & joy is
                nowhere to be found. There literally isn't anything BUT somber reality for me
                right now.
