...
async getFormFields(p){
    ...
    if (p.hasOwnProperty('flat') && (p.flat == true)){
        return(resp.responseText);
    }else{
        try {
...
getRelatedFormsAndMenus(p){
    ...
    return(new Promise(function(toot, boot){
        ...
        // getFormFields on the given schema
        }else{

            let abort = false;
            that.getFormFields({schema: p.schema, flat: true}).catch(function(error){
                abort = true;
                error.message = `${that._className} | getRelatedFormsAndMenus(${p.schema}) | ${error.message}`;
                boot(error);
            }).then(function(formFieldsString){
                if (! abort){
                    try {
                        formDefs[p.schema] = JSON.parse(formFieldsString);
                    }catch(e){
                        abort = true;
                        boot(new noiceRemedyAPIException ({
                            messageType:            'non-ars',
                            message:                `cannot parse getFormFields output? ${e}`,
                            thrownByFunction:       'getRelatedFormsAndMenus',
                            thrownByFunctionArgs:   (typeof(p) !== 'undefined')?p:{}
                        }));
                    }
                    if (! abort){
                        ...
                        // go find menus and forms to get and stack up requests in the pk
                        let pk = [];
                        formDefs[p.schema].forEach(function(fieldDef){
                            if (
                                ...
                                (! (formDefs.hasOwnProperty(fieldDef.limit.source_form)))
                            ){
                                // push a recursive thing here
                            }else if (
                                (fieldDef instanceof Object) &&
                                (fieldDef.hasOwnProperty('limit') &&
                                (fieldDef.limit instanceof Object) &&
                        ...
                        Promise.all(pk).catch(function(error){
                            ...
                            if (! abort){
                                ...
                                /*
                                    LOH 10/14/21 @ 1626
                                    fill in recursive call for a form reference above, and
                                    the getMenu() calls above.

                                    down here, we'll need to unwind the recursive output into the
                                    the returned datastructure.

                                    then just toot it, I guess
                                */
