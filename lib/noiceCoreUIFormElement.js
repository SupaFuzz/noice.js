        ...
        new Promise(function(toot, boot){
            ...
            if (! setValueAbort){
                ...
                if (that.formElement instanceof Element){
                    that.formElement.value = that._value;

                    if (that.cursorToEnd){
                        try {
                            that.formElement.setSelectionRange(that.formElement.value.length,that.formElement.value.length);
                            that.formElement.focus();
                            window.requestAnimationFrame(() => { that.formElement.blur(); });
                        }catch(e){
                            console.log(`${that._className} v${that._version} | value attribute setter | cursorToEnd option failed: ${e}`);
                        }
                    }
